...
def knn_cov_cons (num_to_be_found, space, val_pred, list_col):
    """
  #funzione che cerca gli attributi mancanti per soddisfare il vincolo di copertura
    ...
    :param list_col: lista degli indici delle colonne dei predicati con cui creare l'albero
    :param relax_attributes: oggetto che mantiene tutte le informazioni utili della query iniziale
    :return: numpy.ndarray dei k vicini

  """
    plt.scatter(space[:, list_col[0]], space[:, list_col[1]])
    plt.xlabel("education_num")
    plt.ylabel("hours_per_week")
    plt.title("Plot dati non normalizzati")
    plt.show()
    tree = KDTree(
        space[:, list_col])  # creo l'albero KDtree sullo spazio esterno in cui poi cercare gli elementi più vicini
...
def knn_res_Qind (query, CC, df, list_columns):
    ...
    for indx in range(len(card_AS_Q)):
        ...
        knn_points = knn_cov_cons(num_to_be_found, space, list_val_pred, list_col_pred)
    ...
    print(proximity(relax_attributes, relax_attributes_Qind))
