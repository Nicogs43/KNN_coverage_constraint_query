...
for filename in all_files:
    ...
    if (',' in df['time_norm_data_and_comp_q'].iloc[0]):
        ...
        s = pd.Series([df['query'].iloc[0], filename, df['card_true_tot_Q'].iloc[0], df['card_true_sa_Q'].iloc[0],
                       df['card_tot_res'].iloc[0], df['card_AS_res'].iloc[0], df['card_true_tot_Qind'].iloc[0],
                       df['card_true_sa_Qind'].iloc[0],
                       df['relaxation_degree_res'].iloc[0], df['disparity_index_res'].iloc[0],
                       df['fairness_index_res'].iloc[0],
                       df['relaxation_degree_Qind'].iloc[0], df['disparity_index_Qind'].iloc[0],
                       df['fairness_index_Qind'].iloc[0],
                       df['proximity_Qind'].iloc[0], df['time_read_table'].mean(),
                       list_mean_per_norm_and_comp_q,
                       list_mean_per_time_norm_data, df['time_q_comp'].mean(), df['time_norm_point'].mean(),
                       list_mean_per_time_tree_exec, df['time_exec_Qind'].mean(), df['time_execution'].mean()],
    ...
    s = pd.Series([df['query'].iloc[0], filename, df['card_true_tot_Q'].iloc[0], df['card_true_sa_Q'].iloc[0],
                   ...
                   df['relaxation_degree_Qind'].iloc[0], df['disparity_index_Qind'].iloc[0],
                   df['fairness_index_Qind'].iloc[0],
                   df['proximity_Qind'].iloc[0], df['time_read_table'].mean(), df['time_norm_data_and_comp_q'].mean(),
                   df['time_norm_data'].mean(), df['time_q_comp'].mean(), df['time_norm_point'].mean(),
                   df['time_tree_exec'].mean(), df['time_exec_Qind'].mean(), df['time_execution'].mean()],
                  index=result_csv.columns)
    result_csv = result_csv.append(s, ignore_index=True)
...
writer.save()
#Todo spezzare il dataframe in un altro con solo i valori per la query indotta
