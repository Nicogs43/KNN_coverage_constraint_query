    ...
    for i in json_obj:
        ...
        if curr_min != curr_max:
            for j in range(1, len(i['bins'])):
                new_bins += [abs(((i['bins'][j] - curr_min) / (curr_max - curr_min)) - (
                            (i['bins'][j - 1] - curr_min) / (curr_max - curr_min)))]
...
def distQ_Qnew(data):
    ...
    for i, row in data.iterrows():
        val_newQ_dataset = []
        if row['card_true_tot_Q'] != "CC IS ALREADY SATISFIED" and row[
            'card_true_tot_Q'] != "REWRITING IS NOT POSSIBLE":
            # print(row['newQ'])
...
def main():
    ...
    for i, row in algo.iterrows():
        if (row['card_true_tot_Q'] != 'REWRITING IS NOT POSSIBLE') & (
                row['card_true_tot_Q'] != "CC IS ALREADY SATISFIED"):
            ...
            row['diag_sol'] = calc_min_max_mean_tot_sol(json.loads(row['output_prep']), str(row['solution']),
                                                        row['n_bin'], '')
            row['proximity'] = proximity(json.loads(row['output_prep']), str(row['solution']), row['n_bin'], '')
            row["disparity_index"] = measure_DispInd(row['card_true_sa_Q'], row['card_true_tot_newQ'])
            row['fairness_index'] = measure_FairInd(row['card_true_tot_Q'],row['card_true_tot_newQ'],row['card_true_sa_Q'],row['card_true_sa_newQ']) #aggiunte le due misure (disparity e fairness index) usando le funzioni già implementate
    ...
    algo_new.to_csv('resultsCRBasePI_measures.csv', index=False)
