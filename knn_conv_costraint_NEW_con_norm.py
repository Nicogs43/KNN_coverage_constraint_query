...
# trasformare le relax_attr in liste così che sia più generalizzato
...
def max_min_per_column (df, relax_attr):
    for i in range(len(relax_attr)):
        curr_max = df[relax_attr[i]['attr']].max()
        curr_min = df[relax_attr[i]['attr']].min()
        relax_attr[i]['val_max_col'] = curr_max
        relax_attr[i]['val_min_col'] = curr_min
...
def knn_res_Qind (query, CC, df, list_columns):
    ...
    list_val_pred = [list_val_pred]
    time_norm_point = (time.time() - start_time2)

    start_time3 = time.time()
    card_tot_Q = initial_query_results.shape[0]
    com_orig_query = transform_Q_to_compQ(orig_query)
    time_q_comp = (time.time() - start_time3)

    time_norm_data = []
    time_tree_exec = []
    time_norm_data_and_comp_q = []
    for indx in range(len(card_AS_Q)):
        ...
        time_norm_data.append(time.time() - start_time4)
        time_norm_data_and_comp_q.append(time.time() - start_time5)
        ...
        list_temp.append(res_space)
        # print('res ', knn_points, res_space)
    ...
    # print("Card_tot_res", card_tot_res, card_AS_res)
    end_time = time.time()
...
def main ():
    ...
    # CC = [{'AS': ['race'], 'value': ['Asian-Pac-Islander'], 'num': '300'},{'AS': ['race'], 'value': ['Amer-Indian-Eskimo'], 'num': '100'}]
    # CC = [{'AS': ['race'], 'value': ['Asian-Pac-Islander'], 'num': '80'}]
    # CC = [{'AS': ['race'], 'value': ['Asian-Pac-Islander'], 'num': '900'}]
    # CC = [{'AS': ['race'], 'value': ['Asian-Pac-Islander'], 'num': '900'},{'AS': ['race'], 'value': ['Amer-Indian-Eskimo'], 'num': '500'}]
    # CC = [{'AS': ['race'], 'value': ['Asian-Pac-Islander'], 'num': '600'},{'AS': ['race'], 'value': ['Amer-Indian-Eskimo'], 'num': '600'}]
    # CC = [{'AS': ['sex','marital_status'], 'value': ['Female','Married-civ-spouse'], 'num': '1600'}]
    # CC = [{'AS': ['sex','race'], 'value': ['Female','Black'], 'num': '1600'}]
