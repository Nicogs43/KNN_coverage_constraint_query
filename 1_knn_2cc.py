...
# funzione che crea la complementare della orig_query
def transform_Q_to_compQ_chiara (query: str,CC: object, indice: int):
    # """funzione che presa in input una query la trasforma per renderla la complementare"""
    if query.find('&') == -1 and (query.find('|') == -1):
        query = change_op_to_compQ(query)
    if "&" in query:
...
def get_comp_query_result_chiara (CC: object, indice: int, comp_q: str, df):
    ...
    #     [sens_attr + ' == \'' + CC[indice]['value'][x] + '\'' for x, sens_attr in enumerate(CC[indice]['AS'])])
    #
    # comp_q = '('+com_orig_query+') & (' +condition_str+')'
    # print(comp_q)

    # print(com_orig_query, condition_str)
...
def normalize_data_old(to_be_normalised):
...
def get_new_val_for_pred_old (knn_found, attr_of_pred, relax_attributes):
...
def proximity(relax_attr, relax_attr_Qind):
    ...
    for i in range(len(relax_attr)):
        if (relax_attr[i]['op'] == '<') | (relax_attr[i]['op'] == '<='):
            ...
            point_qind[i] = np.divide(int(relax_attr_Qind[i]['val']) - int(relax_attr[i]['val']),
                                      relax_attr[i]['val_min_col'] - int(relax_attr[i]['val']))
        # print(point_q, point_qind)
    if np.all(point_q != 0):
        print("Il punto della query iniziale non si trova nell'origine. Qualcosa deve essere andato storto!")

    # print(point_q - point_qind)
    # print((np.linalg.norm(point_q - point_qind))/len(relax_attr))


    dist = 0.0
    for ii, ee in enumerate(point_q):
        # print(ii, ee, point_q[ii],point_qind[ii])
        dist += (point_qind[ii])**2
        # dist += (ee-point_q[ii])**2
        # dist += ii**2
        #print(dist)
    dist = math.sqrt(dist)
    # print(dist)
    # print(dist/math.sqrt(len(relax_attr)))
...
#################### useful data manipulation
    ...
    orig_query: str = boolean_op.join([x['attr'] + ' ' + x['op'] + ' ' + x['val_orig'] for x in relax_attributes])
    cond_cc=[(' & ').join(["(complementary['"+sens_attr + "'] == '" + cond['value'][x] + '\')' for x, sens_attr in enumerate(cond['AS'])]) for cond in CC]
    cond_cc2 =[(' & ').join(["(initial_query_results['"+sens_attr + "'] == '" + cond['value'][x] + '\')' for x, sens_attr in enumerate(cond['AS'])]) for cond in CC]
    ...
    complementary, initial_query_results = [xx for _, xx in df.groupby(eval(to_code(orig_query))) ] #else xx[xx['sex']=='Female']
    bb2= time.time()
    complementary_spaces = [complementary.loc[eval(cond_cc[i])] for i in range(0,len(cond_cc)) ]
    time1_2 = time.time()

    # print(initial_query_results, complementary, complementary_spaces)
    ## we count how many individuals for each protrcted group are in the result
    ...
    # trasformo dati in matrice per knn e normalizzo
        ...
        space = complementary_spaces[indx].to_numpy()  # trasformo con numpy il dataframe per poterlo passare alla funzione KDtree# spazio su cui cercare i k vicini
        norm_data = normalize_data(space[:, list_col_pred], relax_attributes)  #0.0089755
        start_time5 = time.time()
        # print('NORM    :' , start_time5 - start_time4)
        ...
        # time_if.append(start_time_tree - start_time6)
    ...
    # print(res_space_no_duplicate)
    # lista con tutti i k vicini trovati per ogni CC da unire evitando di avere doppioni
    # initial_query_results.append(res_space_no_duplicate)
    initial_query_results = pd.concat([initial_query_results, res_space_no_duplicate])
    # initial_query_results.append(res_space_no_duplicate)
    end_time = time.time()
    # print('TEMPOOOOOOOO ', end_time - time_6_1)
    # print(initial_query_results, len(initial_query_results))
    ...
    # time_norm_data =start_time5 - start_time4
    # time_tree_exec= end_time_tree - start_time_tree
    # time_res_no_duplicates= time_6_1 - end_time_tree

    time_finalres = (end_time- end_time_for)
    time_exec = end_time - start_time
    # time_for_CC = end_time_for-start_time_for
    # print(time_finalres)
    print('TEMPOOOOOOOO ', time_exec, 'tempi df e as', time_for_df_cc, time_for_df, time_query_as, 'time_final_res:',  time_finalres)


    card_AS_res: List[int] = counts_Value_col(initial_query_results, CC)
    card_tot_res = initial_query_results.shape[0]
    # scatterplot_2(df, res_space_no_duplicate,relax_attributes[0]['attr'], relax_attributes[1]['attr'],int(relax_attributes[0]['val']),int(relax_attributes[1]['val']), 'data_f_2')
    ...
    # print("Card_Tot_Qind",card_tot_Qind, induced_Q, time_norm_data)
...
def main ():
    ...
    # CC = [{'AS': ['sex'], 'value': ['Female'], 'num': '10600'}]
